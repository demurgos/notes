...
I had more success using `nvidia-xrun`, but I am not sure what needs to be changed compared to the previous section.

I have the following config in `~/.nvidia-xinitrc`:
...
Add the following block to the SDDM X setup

`sudo nano /usr/share/sddm/scripts/Xsetup`

<!--
xrandr --newmode "1680x1050s" $(cvt 1680 1050 | grep Modeline | cut -d' ' -f 3-)
-->

```
# Set resolution for the external VGA monitor
xrandr --addmode DP-1 "1680x1050"
xrandr --output DP-1 --mode "1680x1050"
```

Add the following to `~/.config/autostart-scripts/external_monitor.sh` to configure the resolution when KDE starts:
Then make it executable with `chmod +x ./external_monitor.sh`
...
In `/etc/default/grub`, make sure the options are as follow:

```
GRUB_CMDLINE_LINUX_DEFAULT="audit=0 loglevel=3 acpi_rev_override=1 intel_iommu=on"
```

acpi_rev_override: ACPI support (for nvidia-xrun)
intel_iommu: VFIO support (GPU passthrough)
